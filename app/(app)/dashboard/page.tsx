...
interface TeamMember {
  ...
  progress: number;
  badges: { id: string; badge_icon: string | null }[];
}
    ...
    const init = async () => {
      ...
      if (allAnnouncements?.length) {
        ...
        const unread = allAnnouncements.filter(
          (ann: { id: string }) => !readIds.includes(ann.id)
        );
      ...
      const validIds = processes?.map((p: { id: string }) => p.id) || [];

      let completions: { user_id: string; process_id: string }[] = [];
      ...
      const myProgress =
        totalProcesses > 0
          ? Math.round((myCompletions.length / totalProcesses) * 100)
          : 0;
      ...
      const teamWithProgress: TeamMember[] =
        profiles?.map((member: any) => {
          ...
          const percentage =
            totalProcesses > 0
              ? Math.round((memberCompletions.length / totalProcesses) * 100)
              : 0;

          const memberBadges =
            processes?.filter((p: any) =>
              memberCompletions.some((c) => c.process_id === p.id)
            ) || [];
      ...
      teamWithProgress.sort((a, b) => b.progress - a.progress);
